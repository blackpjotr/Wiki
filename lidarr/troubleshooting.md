...
The more we know, the easier it's to help you.
...
In the event of a migration issue there isn't much you can do immediately, if the issue is specific to you (or there aren't yet any posts), please create a post on [our subreddit](https://reddit.com/r/lidarr) or swing by our [discord](https://lidarr.audio/discord), if there are others with the same issue, then rest assured we're working on it.

> Please ensure you didn't try to use a database from `nightly` on the stable version. Branch hopping is ill-advised.{.is-info}
...
While mapped network drives like `X:\` are convenient, they aren’t as reliable as UNC paths like `\\server\share` and they’re also not available before login. Setup and your download clients so that they use UNC paths as needed. If your library is on a share, you’d make sure your root folders are using UNC paths. If your download client sends to a share, that's where you’ll need to configure UNC paths since gets the download path from the download client. It's fine to keep your mapped network drives to use yourself, just don’t use them for automation.
...
- You need it to be syncing the whole time it's downloading. And you need that local sync destination to be able to be hard links when \*Arr import, which means same file system and look like it.
